...
namespace Nanook.NKit
{
    public class FstFolder
    {
        ...
        {
            Folders = new List<FstFolder>();
            Files = new List<FstFile>();
            Parent = parent;
        }
    ...
    internal class ConvertFile
    {
        ...
        public long NewSize => IsJunk ? FstFile.Length % 4 : FstFile.Length;
        public Gap Gap { get; internal set; }
        public long GapLength { get; internal set; }
        public bool HasGap => GapLength != 0;
    ...
    public class FstFile
    {
        ...
        {
            return new FstFile(Parent) { PartitionId = PartitionId, DataOffset = DataOffset, Length = Length, Name = Name, Offset = Offset, IsNonFstFile = IsNonFstFile, OffsetInFstFile = OffsetInFstFile };
        }
    ...
    public class FileSystem
    {
        ...
        public FstFile[] Files => recurseFolders(Root, new List<FstFile>()).OrderBy(a => a.Offset).ToArray();
        ...
        public static FileSystem Parse(byte[] fstData, long fstOffset, string id, bool isGc)
        {
            ...
            FstFile ff = new FstFile(null) { Name = "fst.bin", DataOffset = fstOffset, Offset = NStream.DataToOffset(fstOffset, !isGc), IsNonFstFile = true, Length = fstData.Length };
