    ...
    public class NDisc : IDisposable
    {
        ...
        public IEnumerable<IWiiDiscSection> EnumerateSectionsFix(bool generateUpdateFiller, bool generateOtherFiller, bool forceFillerJunk)
        {
            ...
            foreach (WiiPartitionInfo part in hdr.Partitions) //already sorted
            {
                ...
                {
                    partSec = new WiiPartitionSection(NStream, (WiiDiscHeaderSection)NStream.DiscHeader, NStream, discOffset);
                }
        ...
        public void SaveMainDol(string outPath)
        {
            ...
            dolHdr.CopyTo(dol, 0);
            NStream.Read(dol, dolHdr.Length, dol.Length - dolHdr.Length);
            string dolName = Path.Combine(outPath, string.Format("main[{0}][{1}][{2}].dol", NStream.Id8, hdr.ReadUInt32B(0x420).ToString("X8"), Crc.Compute(dol).ToString("X8")));
        ...
        public ExtractResult ExtractBasicInfo()
        {
            ...
            if (IsGameCube)
            {
                MemorySection bi2bin = MemorySection.Read(NStream, 0x2000);
                region = (Region)bi2bin.ReadUInt32B(0x18);
            }
            else
            {
                region = (Region)Header.ReadUInt32B(0x4e000);
            }
        ...
        public ExtractResult ExtractFiles(Func<ExtractedFile, bool> filter, Action<Stream, ExtractedFile> extract)
        {
            if (IsGameCube)
            {
                return ExtractFilesGc(filter, extract);
            }
            else if (NStream.IsNkit)
            {
                return ExtractFilesWiiNkit(filter, extract);
            }
            else
            {
                return ExtractFilesWii(filter, extract);
            }
        }
        ...
        {
            long mlt = IsGameCube ? 1L : 4L;

            long srcPos = 0;
            List<FstFile> files = new List<FstFile>();

            MemorySection bootbin = (IsGameCube || NStream.IsNkit) ? hdr : MemorySection.Read(inStream, 0x440);
            ...
            List<FstFile> fstFiles = FileSystem.Parse(fstbin, null, id, IsGameCube)?.Files?.OrderBy(a => a.Offset)?.ThenBy(a => a.Length)?.ToList();
            if (fstFiles == null)
            {
            ...
            List<ExtractedFile> exfiles = files.OrderBy(a => a.Offset).ThenBy(a => a.Length).Select(a => new ExtractedFile(IsGameCube ? DiscType.GameCube : DiscType.Wii,
                                 NStream.Id8,
                                 IsGameCube ? null : id,
                                 a.DataOffset,
                                 a.Length,
        ...
        {
            return extractFiles(NStream.Id, (MemorySection)Header, NStream, filter, extract);
        }
        ...
        public ExtractResult ExtractFilesWii(Func<ExtractedFile, bool> filter, Action<Stream, ExtractedFile> extract)
        {
            ...
            foreach (IWiiDiscSection s in EnumerateSectionsFix(false, true, false))
            {
                ...
                else if (s is WiiPartitionSection)
                {
                    ...
                    {
                        exFile = new ExtractedFile(IsGameCube ? DiscType.GameCube : DiscType.Wii,
                            NStream.Id8, null, ps.Header.DiscOffset, ps.Header.Size, "", ps.Header.Id + "hdr.bin", ExtractedFileType.WiiDiscItem);
                        if (filter(exFile))
                        {
        ...
        public ExtractResult ExtractRecoveryFilesGc()
        {
            ...
            using (CryptoStream bw = new CryptoStream(ByteStream.Zeros, crc, CryptoStreamMode.Write))
            {
                ...
                if ((storeType = storeRecoveryFile(Settings.RedumpAppldrCrcs.Contains(appLdrCrc), tmpFullName, fn, Settings.RecoveryFilesPath, Settings.OtherRecoveryFilesPath, false)) != 0)
                {
            ...
            if ((storeType = storeRecoveryFile(Settings.RedumpFstCrcs.Contains(crc.FullCrc()), tmpFullName, fn, Settings.RecoveryFilesPath, Settings.OtherRecoveryFilesPath, false)) != 0)
            {
                result.Add(new ExtractRecoveryResult() { FileName = fn, Extracted = true, Type = PartitionType.Other, IsNew = storeType == 2, IsGameCube = true });
            }
        ...
        public ExtractResult ExtractRecoveryFilesWii()
        {
            ...
            long imageSize = NStream.RecoverySize; //for Wii: pHdr.PartitionDataLength
        ...
        private ExtractResult createExtractResult(Region region, ExtractRecoveryResult[] recovery)
        {
            ...
            result.Id = NStream.Id8;
            result.DiscType = IsGameCube ? DiscType.GameCube : DiscType.Wii;
            result.Title = NStream.Title;
        ...
        internal long WriteRecoveryPartitionData(Stream crcStream, bool unscrub, WiiPartitionSection ps, int channelNo, out string tempFileName, out string fileName, out NStream output)
        {
            ...
            {
                fileName = string.Format("{0}_{1}_{2}_{3}_", NStream.Id8, channelNo.ToString().PadLeft(2, '0'), SourceFiles.CleanseFileName(ps.Header.Id).PadRight(4), ps.Header.IsKorean ? "K" : "N");
            }
        ...
        internal int WriteRecoveryPartitionFiller(Stream crcStream, JunkStream junk, long pos, bool isUpdate, bool isNkit, WiiFillerSection fs, NStream target, string tmpFileName, ref string fileName, Crc crc, bool logAsDetail)
        {
            ...
            foreach (WiiFillerSectionItem fi in fs.Sections)
            {
                ...
                {
                    int len = (int)Math.Min(0x8000L, fi.Size - i);
            ...
            if (isUpdate)
            {
                redump = Settings.RedumpUpdateCrcs.Contains(crc.Value);
            }
            else
            {
                redump = Settings.RedumpChannels.FirstOrDefault(a => fName.StartsWith(a.Item1)) != null;
            }
