...
namespace Nanook.NKit
{
    ...
    internal class HeaderBruteForcer
    {
        ...
        private readonly uint[] _updateCrcs;
        private readonly SortedList<uint, bool> _checkCrcs;

        private readonly uint[] _origHeaderCrcs;

        private uint _origHeaderCrc;
        private uint _origUpdateCrc;
        private readonly int _origRegion;
        private readonly byte[] _origRegionData;
        private readonly List<headerCrc> _hdrs;
        private readonly List<regionCrc> _regionData;
        ...
        public HeaderBruteForcer(uint[] updateCrcs, SortedList<uint, bool> checkCrcs, Tuple<byte[], int[]>[] regionData, params byte[][] headers)
        {
            ...
            for (int h = 0; h < _hdrs.Count; h++)
            {
                for (int i = 0; i < regionData.Length; i++)
                {
                    ...
                    {
                        _regionData.Add(new regionCrc() { Crc = 0, Header = _hdrs[h], RegionData = regionData[i].Item1, Region = regionData[i].Item2[r] });
                    }
                }
            }
