...
namespace Nanook.NKit
{
    ...
    internal class WiiPartitionGroupEncryptionState
    {
        private class block
        {
            ...
            {
                return string.Format("Index:{0}, Offset:{1}, {2}, {3}, {4}", Index.ToString("X"), Offset.ToString("X"), IsUsed ? "Used" : "NotNused", IsScrubbed ? "Scrubbed" : "NotScrubbed", IsDirty ? "Dirty" : "Clean");
            }
        }
        ...
        public byte[] Encrypted => ensureEncrypted();
        public byte[] Decrypted => ensureDecrypted();
        ...
        internal bool FastHashIsValid()
        {
            ...
            bool anyInvalid = _blocks.AsParallel().Any(b =>
            {
                if (b.IsUsed)
                {
                    ...
                    if (!Decrypted.Equals(b.Offset + 0x26c, 20, 0) || !Decrypted.Equals(b.Offset + 0x320, 32, 0) || !Decrypted.Equals(b.Offset + 0x3e0, 32, 0))
                    {
                        return true;
                    }

                    //Test H1
                    if (!Decrypted.Equals(b.Offset + 0x280, b.H1Table.Bytes, 0, 0xA0))
                    {
                        return true;
                    }

                    //Test H2
                    if (b != b1 && !Decrypted.Equals(b.Offset + 0x340, b1.H2Table.Bytes, 0, 0xA0))
                    {
                        return true;
                    }
                }
        ...
        public int ScrubbedBlocks => _blocks.Count(a => a.IsScrubbed);
