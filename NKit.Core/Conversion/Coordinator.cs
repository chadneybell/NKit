...
namespace Nanook.NKit
{
    ...
    internal class Coordinator
    {
        ...
        public HandledException Exception => _readerException == null || _writerFirst ? _writerException : _readerException;
        public long OutputSize => _processSize;
        ...
        public HandledException SetReaderException(Exception ex, string message, params string[] args)
        {
            ...
            {
                return SetReaderException((HandledException)ex);
            }

            return SetReaderException(new HandledException(ex, message, args));
        }
        ...
        public HandledException SetWriterException(Exception ex, string message, params string[] args)
        {
            if (ex is HandledException)
            {
                return SetWriterException((HandledException)ex);
            }

            return SetWriterException(new HandledException(ex, message, args));
        }
        ...
        public void ReaderCheckPoint1PreWrite(string aliasJunkId, uint nkitSourceCrc)
        {
            ...
            if (Started != null)
            {
                Started(this, new StartedEventArgs(_processSize, aliasJunkId));
            }
        }
        ...
        public void WriterCheckPoint3ApplyPatches(NCrc crcsPatches, bool isRecoverable, uint validationCrc, ChecksumsResult checksums, bool verifyIsWrite, string resultMessage)
        {
            ...
            WriterCheckPoint3ApplyPatches(crcsPatches, isRecoverable, validationCrc, checksums.Crc, verifyIsWrite, resultMessage);
        }
        ...
        public void WriterCheckPoint3ApplyPatches(NCrc crcsPatches, bool isRecoverable, uint validationCrc, uint verifiedCrc, bool verifyIsWrite, string resultMessage)
        {
            ...
            Patches = crcsPatches ?? _crcs; //the reader might be the patch issuer and the writer might be a plain iso writer
        }
        ...
        public void ReaderCheckPoint3Complete()
        {
            ...
            if (Completed != null)
            {
                Completed(this, new CompletedEventArgs(Patches?.FullCrc(true) ?? 0, Patches?.FullCrc(false) ?? 0, _processSize, _header, _resultMessage, _validationCrc, _verifiedCrc, _verifyIsWrite, _isRecoverable, _md5, _sha1));
            }
        }
