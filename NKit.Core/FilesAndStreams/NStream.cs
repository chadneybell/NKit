...
namespace Nanook.NKit
{
    ...
    public class NStream : Stream
    {
        ...
        public bool HeaderRead => _headerRead;
        public string Id => DiscHeader?.ReadString(0, 4);
        public string Id6 => DiscHeader?.ReadString(0, 6);
        public string Id8 => DiscHeader == null ? null : string.Concat(Id6, DiscNo.ToString("X2"), Version.ToString("X2"));

        public int DiscNo => DiscHeader?.Read8(6) ?? 0;
        public int Version => DiscHeader?.Read8(7) ?? 0;

        public string Title => DiscHeader?.ReadStringToNull(0x20, 0x60);
        internal MemorySection DiscHeader { get; set; }
        internal JunkStream JunkStream { get; set; }

        internal bool JunkGeneration { get; set; }

        internal virtual Stream BaseStream => _stream;

        public bool IsNkit => _isNkit;
        public bool IsNkitUpdateRemoved => _isNkitUpdateRemoved;
        public bool IsGcz => _isGcz;
        public bool IsIso => _isIso;
        public bool IsWii => _isWii;
        public bool IsGameCube => _isGamecube;
        public bool IsWbfs => _isWbfs;
        public bool IsIsoDec => _isIsoDec;
        ...
        public override long Length => _readLength;  //length that will be read (size read from wbfs, gcz, iso.dec containers) 

        public long ImageSize => _imageSize;  //Same as above, if nkit it refers to the converted size

        public long RecoverySize => lenCalc(_imageSize);  //same as above, but in the case of truncated isos, it's the full recovered size

        public long SourceSize => _stream.Length;  //size of the source file on the disc in which ever format
        ...
        internal bool Initialize(bool readHeader)
        {
            ...
            try
            {
                if (readHeader)
                {
                    ...
                    if (id == "WBFS")
                    {
                        uint hdSecSize = readUInt32B(); //n_hd_sec
                        long pos = (uint)(1 << read8()) + 0x100;
                        _clusterSize = 1 << read8();
                        ...
                        for (int i = 0; i < clusters; i++)
                        {
                            ...
                            else if (pointer * _clusterSize < fileSize)
                            {
                                blank += blankTmp;
                                blankTmp = 0;
                                maxPointer = Math.Max(maxPointer, pointer * _clusterSize);
                            }
                    ...
                    else if ((id == "WII5" || id == "WII9") && !_isGamecube) //IsoDec :(
                    {
                        ...
                        string discId = readString(4);
                        byte[] md5 = read(16);
                        int partitions = (int)readUInt32L();
                        for (int i = 0; i < partitions; i++)
                        {
                            var part = new //throw away
                            {
                                DataOffset = (long)((ulong)readUInt32L() << 2),
                                DataSize = (long)((ulong)readUInt32L() << 2),
                                PartitionOffset = (long)((ulong)readUInt32L() << 2),
                                PartitionEndOffset = (long)((ulong)readUInt32L() << 2),
                                PartitionKey = read(16)
                            };
        ...
        {
            JunkStream = new JunkStream(Id, DiscNo, RecoverySize);
        }
        public void ChangeJunk(string id)
        {
            JunkStream = new JunkStream(id, DiscHeader.Read8(6), _imageSize);
        }
        public void ChangeJunk(long baseAddress, string id, int discNo, long length)
        {
            ...
            JunkStream = new JunkStream(id, discNo, length);
        }
        ...
        private void setDiscHeader(bool autodetect, long[] isoDecParts)
        {
            if (autodetect)
            {
                ...
                x.CopyTo(h, 0);
                Read(h, x.Length, h.Length - x.Length);
                DiscHeader = new MemorySection(h);

            }
            else
            {
                DiscHeader = MemorySection.Read(this, _isWii ? _HeaderSizeWii : _HeaderSizeGc);
            }

            if (IsWii)
            {
                DiscHeader = new WiiDiscHeaderSection(DiscHeader);
                ((WiiDiscHeaderSection)DiscHeader).IsoDecPartitions = isoDecParts;
            }
            _isNkit = DiscHeader.ReadString(0x200, 4) == "NKIT";
            _isNkitUpdateRemoved = DiscHeader.ReadUInt32B(0x218) != 0; //update not 0means the update partition has been removed
            if (_isNkit)
            {
                _imageSize = DiscHeader.ReadUInt32B(0x210) * (_isWii ? 4L : 1L);
            }

            _position = DiscHeader.Size;
        }
        ...
        public static long FullSizeGameCube => 0x57058000L;
        public static long FullSizeWiiRvtr => 0x118940000L;
        public static long FullSizeWiiOversized => 0x118248000L;
        public static long FullSizeWii5 => 0x118240000L;
        public static long FullSizeWii9 => 0x1FB4E0000L;
        ...
        public long RealPosition(long position)
        {
            if (_isWbfs)
            {
                ...
                return _clusterTable[clusterIdx] * _clusterSize + inClusterOffset;
            }
            else if (_isIsoDec)
            {
                ...
                return _clusterTable[clusteroffset] * _IsoDecMultiply + position % _clusterSize;
            }
            else
            {
                return position;
        ...
        public override int Read(byte[] buffer, int offset, int size)
        {
            ...
            if (size != 0 && r == 0)
            {
                throw new HandledException(string.Format("NStream.Read - No data read at Position {0} ({1}) - Requested {2}", Position.ToString("X"), RealPosition(Position).ToString("X"), size.ToString()));
            }
            ...
            try
            {
                if (_isGcz)
                {
                    ...
                    for (int i = clusteroffset; i < clusters; i++)
                    {
                        ...
                        {
                            int clusterSize = (int)(((i + 1) != clusters ? _clusterTable[i + 1] : _stream.Length) - _clusterTable[i]);
                            int rd;
                ...
                else if (_isWbfs)
                {
                    ...
                    while (size > 0)
                    {
                        ...
                        {
                            long pos = clusterWbfsIdx * _clusterSize + inClusterOffset;
                    ...
                    for (int i = clusteroffset; i < clusters; i++)
                    {
                        ...
                        if (_clusterTable[i] == 0xffffffff)
                        {
                            ...
                            else
                            {
                                if (deferedJunk == null)
                                {
                                    JunkStream.Position = _junkBaseOffset == 0 ? _position : OffsetToData(_position - _junkBaseOffset);
                                    JunkStream.Read(buffer, offset, len);
                                }
                                else
                                {
                        ...
                        else
                        {
                            if (firstSeek)
                            {
                                long pos = _clusterTable[i] * _IsoDecMultiply + _position % _clusterSize;
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            ...
            try
            {
                ...
                _stream.Position = RealPosition(_position);
                return _position;
            }
            catch (Exception ex)
            {
        ...
        public override bool CanWrite => _isWbfs || _isIsoDec ? false : _stream.CanWrite;
        ...
        private byte read8() { return read(1)[0]; }
        private ushort readUInt16B() { return bigEndian(BitConverter.ToUInt16(read(2), 0)); }
        private uint readUInt32L() { return littleEndian(BitConverter.ToUInt32(read(4), 0)); }
        private uint readUInt32B() { return bigEndian(BitConverter.ToUInt32(read(4), 0)); }
        public string readString(int length) { return Encoding.ASCII.GetString(read(length)); }
