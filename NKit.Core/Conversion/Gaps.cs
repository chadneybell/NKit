...
namespace Nanook.NKit
{
    ...
    internal class Gap
    {
        ...
        private readonly List<GapBlock> _blocks;
        private GapBlock _current;
        private readonly long _gapLength;
        ...
        private int blockCompare(long nulls, byte[] junk, long junkLength, ScrubManager scrub, byte[] buff, int bStart, int bEnd, long srcPos, bool isGapStart, out byte? scrubByte)
        {
            ...
            if (bTst < bEnd)
            {
                bTst += scrub.IsScrubbed(srcPos + bStart, buff, bTst, bEnd - bStart, out byte scrubB, out scrubDecrypted);
                if (bTst == bEnd)
                {
                    scrubByte = scrubB;
                }
